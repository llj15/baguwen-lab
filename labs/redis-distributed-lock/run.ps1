...
try {
    Write-Host "=========================================="
    Write-Host "  Redis distributed lock lab run"
    Write-Host "=========================================="
    ...
    Write-Host "`n[2/4] Building and running experiments..."
    ...
    Invoke-Compose @("run", "--rm", "--no-deps", "analysis", "python", "/app/scripts/verify_results.py", "/data", "--baseline", "/baseline")
}
finally {
