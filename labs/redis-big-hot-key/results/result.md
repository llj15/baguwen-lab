# Redis 大 Key 与热点 Key 实验结果

本文档由真实的 Docker Compose 实验运行生成。数据集由实验程序在运行时构造并写入 Redis，随后通过 `MEMORY USAGE`、`HLEN`、`LLEN`、`HGETALL`、`HSCAN` 以及真实的 `GET` 访问负载从 Redis 中测量。仓库中记录的数值不是手写夹具，也不是为了结论人为编造的数据。

## 数据集

| 维度 | 数值 |
| --- | ---: |
| 普通字符串 key 数量 | 20000 |
| 普通字符串 value 大小 | 128 bytes |
| 大字符串 value 大小 | 8388608 bytes |
| 大 Hash 字段数 | 50000 |
| 大 List 元素数 | 50000 |
| 热点 Key 访问空间 | 10000 keys |
| 热点 Key 请求数 | 100000 |
| 热点 Key 倾斜比例 | 60% 请求集中到一个逻辑 key |

## 大 Key 实验结果

![大 Key 内存占用](big-key-memory.png)

| 场景 | 测量值 |
| --- | ---: |
| 普通字符串平均内存 | 224 bytes |
| 大字符串内存 | 10485816 bytes |
| 大字符串逻辑长度 | 8388608 bytes |
| 大 Hash 内存 | 6786544 bytes |
| 大 Hash 字段数 | 50000 |
| 大 List 内存 | 3679800 bytes |
| 大 List 元素数 | 50000 |

大字符串、大 Hash 和大 List 都是合法的 Redis 对象，但它们把远高于普通 key 的内存占用和命令处理量集中到了单个 key 上。大 Key 的核心风险不在于 Redis 不能存储它，而在于单个命令、单个 key 的所有权和迁移成本会变得过重。

完整读取 Hash 时，`HGETALL` 在一次命令中返回了 `50000` 个字段。游标扫描同样读取了 `50000` 个字段，但把工作拆成了 `100` 次调用。耗时会受宿主机影响，但命令形态是稳定的：`HGETALL` 会产生一次大响应，`HSCAN` 则允许调用方分片处理。

Hash 拆分方案保留了全部 `50000` 个字段，并把最大 bucket 降到 `500` 个字段，数据分布在 `100` 个 key 上。拆分不会减少总数据量，但会降低单个 key 和单条命令承受的最大压力。

## 热点 Key 实验结果

![热点 Key 访问分布](hot-key-distribution.png)

| 场景 | 访问量最高的 key | 最高访问次数 | 最高访问占比 |
| --- | --- | ---: | ---: |
| 均匀访问 | `item:7732` | 23 | 0.0002 |
| 单热点 key | `item:0000` | 60000 | 0.6000 |
| 热点 key 读副本分片 | `item:hot:00` | 3750 | 0.0375 |

均匀访问负载把读请求分散到 `9999` 个 key 上，因此没有单个 key 占据主导。热点 Key 负载把 `100000` 次请求中的 `60000` 次打到 `item:0000`，从访问计数上直接证明了流量倾斜问题。

读副本分片方案保持同一个逻辑热点数据不变，但把读取分散到 `16` 个物理 key 上。最热的副本收到 `3750` 次读取，而不是原始热点 key 的 `60000` 次读取，从而降低单个 Redis key 的压力。

![缓解效果汇总](summary.png)

## 治理前后数据对比

下表只列出本实验真实执行并测量过的治理方案。没有在实验中执行的生产建议不会在这里给出前后数据。

| 治理点 | 治理前 | 治理后 | 变化 |
| --- | ---: | ---: | --- |
| 大 Hash 单 key 字段数 | 50000 fields/key | 500 fields/key | 单 key 最大字段数降低 100.0 倍，降幅 99.0% |
| 大 Hash key 数量 | 1 key | 100 keys | 总字段数仍为 50000，压力从单 key 分散到多个 key |
| Hash 全量读取形态 | `HGETALL` 1 次返回 50000 fields | `HSCAN` 100 次读完 50000 fields | 平均每次扫描约 500 fields，单次响应规模降低约 100.0 倍 |
| 热点 key 最高读取次数 | 60000 reads/key | 3750 reads/key | 单物理 key 最高读压力降低 16.0 倍 |
| 热点 key 最高访问占比 | 0.6000 | 0.0375 | 最高访问占比降低 93.75% |

这些对比说明：大 Key 的治理目标不是减少总数据量，而是降低单 key、单命令的最大压力；热点 Key 的治理目标不是让访问量消失，而是把同一个逻辑热点分摊到多个物理承载点。

## 解决方案与适用边界

### 大 Key 解决方案

| 问题 | 推荐方案 | 本实验是否验证 | 适用边界 |
| --- | --- | --- | --- |
| 单个 Hash/List/Set/ZSet 元素过多 | 按业务维度拆成多个小 key，例如按 bucket、页号、时间片拆分 | 是，`50000` 个字段拆到 `100` 个 key，最大 bucket 为 `500` 个字段 | 需要业务能接受多 key 读写，查询时可能要合并多个 bucket |
| 一次命令返回过大 | 用 `HSCAN`、`SSCAN`、`ZSCAN`、分批 `LRANGE` 替代全量读取 | 是，`HSCAN` 把 `50000` 个字段拆成 `100` 次调用 | 适合后台遍历和分页读取，不适合必须强一致返回全量数据的接口 |
| 单个大字符串过大 | 拆分为分片 key，或把大对象放对象存储/数据库，Redis 只存索引和热点摘要 | 否，本实验只测量了 8 MiB 字符串的内存占用 | 适合图片、长文本、大 JSON 等对象；需要处理分片一致性和清理逻辑 |
| 删除大 Key 阻塞主线程 | 使用 `UNLINK` 异步删除，或后台分批删除集合元素后再删除空 key | 否 | 适合线上清理；删除前应先限速和灰度，避免瞬时内存/CPU 抖动 |
| 发现大 Key | 使用 `MEMORY USAGE`、`redis-cli --bigkeys`、离线 RDB 分析、业务侧 key 规模指标 | 部分验证，本实验使用 `MEMORY USAGE` 采集内存 | `--bigkeys` 会扫描 keyspace，线上执行要控制时间窗口和频率 |

大 Key 的治理优先级是：先识别，再限制增长，最后拆分或迁移。面试回答时可以概括为“避免全量命令、拆结构、异步删、监控发现、从数据模型上限制单 key 规模”。

### 热点 Key 解决方案

| 问题 | 推荐方案 | 本实验是否验证 | 适用边界 |
| --- | --- | --- | --- |
| 读热点集中到单个 key | 读副本分片，把一个逻辑热点数据复制到多个物理 key，客户端按随机或哈希读取 | 是，单热点从 `60000` 次读取降到单副本最高 `3750` 次读取 | 适合读多写少；写入时要同步更新多个副本或接受短暂不一致 |
| 极高频读请求 | 本地缓存、进程内缓存、Caffeine/Guava、Nginx/CDN 缓存，Redis 作为二级缓存 | 否 | 适合允许短 TTL 或最终一致的热点数据；要防止本地缓存雪崩和脏读 |
| 写热点，例如点赞数、库存扣减、计数器 | 分片计数、队列削峰、异步聚合、定时合并，必要时用 Lua 保证单分片原子性 | 否 | 不适合简单复制 key；写热点要解决聚合准确性、幂等和延迟 |
| 热点突然出现 | 热点探测后动态加副本、短 TTL 本地缓存、限流和降级 | 否 | 适合突发流量；需要网关、代理或业务埋点能识别 top key |
| 发现热点 Key | 统计业务访问日志、代理层 top key、Redis 客户端埋点、慢日志和监控平台 | 部分验证，本实验直接记录了访问计数分布 | Redis 原生不直接提供完整实时 top key，通常要在客户端或代理层做统计 |

热点 Key 的治理优先级是：先确认是读热点还是写热点。读热点优先用本地缓存和读副本分片；写热点不能简单复制 key，通常要做分片写入、削峰和异步聚合。

## 实验结论

- 大 Key 会带来更高的单 key 内存占用和更大的单命令返回载荷；结构拆分和游标扫描可以降低单条命令的最大处理量。
- 热点 Key 本质是流量分布问题；判断依据应重点看最高访问占比，而不只是看延迟。
- 读副本分片可以降低读热点 Key 对单个 Redis key 的压力；写热点 Key 通常还需要计数器分片、异步聚合或其他写入侧拆分方案。
- 耗时数据只作为观察项记录。校验器检查的是稳定不变量：生成数据规模、Redis 基数、内存下限、访问分布和缓解效果。
