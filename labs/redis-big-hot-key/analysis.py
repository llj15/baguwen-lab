...
def markdown_report(data: dict[str, Any]) -> str:
    ...
    hot = scenario(data, "redis_hot_key", "single_hot_key_access")
    sharded = scenario(data, "redis_hot_key", "sharded_hot_key_access")
    split_bucket_reduction = big_hash["fields"] / split_hash["max_bucket_fields"]
    split_bucket_reduction_pct = (1 - split_hash["max_bucket_fields"] / big_hash["fields"]) * 100
    hscan_avg_fields = hash_read["hscan_fields"] / hash_read["hscan_iterations"]
    scan_payload_reduction = hash_read["hgetall_fields"] / hscan_avg_fields
    hot_read_reduction = hot["top_count"] / sharded["max_copy_gets"]
    hot_share_reduction_pct = (1 - sharded["top_ratio"] / hot["top_ratio"]) * 100
...
下表只列出本实验真实执行并测量过的治理方案。没有在实验中执行的生产建议不会在这里给出前后数据。

| 治理点 | 治理前 | 治理后 | 变化 |
| --- | ---: | ---: | --- |
| 大 Hash 单 key 字段数 | {big_hash['fields']} fields/key | {split_hash['max_bucket_fields']} fields/key | 单 key 最大字段数降低 {split_bucket_reduction:.1f} 倍，降幅 {split_bucket_reduction_pct:.1f}% |
| 大 Hash key 数量 | 1 key | {split_hash['bucket_count']} keys | 总字段数仍为 {split_hash['total_fields']}，压力从单 key 分散到多个 key |
| Hash 全量读取形态 | `HGETALL` 1 次返回 {hash_read['hgetall_fields']} fields | `HSCAN` {hash_read['hscan_iterations']} 次读完 {hash_read['hscan_fields']} fields | 平均每次扫描约 {hscan_avg_fields:.0f} fields，单次响应规模降低约 {scan_payload_reduction:.1f} 倍 |
| 热点 key 最高读取次数 | {hot['top_count']} reads/key | {sharded['max_copy_gets']} reads/key | 单物理 key 最高读压力降低 {hot_read_reduction:.1f} 倍 |
| 热点 key 最高访问占比 | {hot['top_ratio']:.4f} | {sharded['top_ratio']:.4f} | 最高访问占比降低 {hot_share_reduction_pct:.2f}% |

这些对比说明：大 Key 的治理目标不是减少总数据量，而是降低单 key、单命令的最大压力；热点 Key 的治理目标不是让访问量消失，而是把同一个逻辑热点分摊到多个物理承载点。
